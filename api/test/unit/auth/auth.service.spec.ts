...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    fakeRoleService = {
      findById: jest.fn().mockResolvedValue(customerRole),
    };
  ...
  describe('register', () => {
    ...
    it('should throw error if user exists', async () => {
      (fakeUserService.findByEmail as jest.Mock) = jest.fn().mockResolvedValue(newUser);
  ...
  describe('login', () => {
    it('should success', async () => {
      fakeUserService.findByEmail = () => {
        return Promise.resolve({ ...newUser, roles: [] });
      };
    ...
    it('should throw error if wrong password', async () => {
      (fakeUserService.comparePassword as jest.Mock) = jest.fn().mockResolvedValue(false);
      await expect(
        service.login({
          email: 'email',
          password: 'password',
